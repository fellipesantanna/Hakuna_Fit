...
// Tipo auxiliar: linha crua do Supabase
type DbWorkoutExercise = {
    ...
    // join
    exercises?:
    | { name: string; equipment: string | null; notes: string | null }
    | { name: string; equipment: string | null; notes: string | null }[]
    | null
}

// Tipo que o front usa (com nome vindo do join)
...
/* =========================
   UPDATE
========================= */
export async function updateWorkoutExercise(id: string, payload: Partial<WorkoutExercise>) {
    const updatePayload: any = {}

    if ("reps" in payload) updatePayload.reps = payload.reps ?? null
    if ("weight" in payload) updatePayload.weight = payload.weight ?? null

    if ("timeHours" in payload) updatePayload.time_hours = payload.timeHours ?? null
    if ("timeMinutes" in payload) updatePayload.time_minutes = payload.timeMinutes ?? null
    if ("distance" in payload) updatePayload.distance = payload.distance ?? null

    if ("durationMinutes" in payload) updatePayload.duration_minutes = payload.durationMinutes ?? null
    if ("durationSeconds" in payload) updatePayload.duration_seconds = payload.durationSeconds ?? null

    if ("completed" in payload) updatePayload.completed = payload.completed ?? false

    const { error } = await supabase.from("workout_exercises").update(updatePayload).eq("id", id)
    if (error) throw error
}
...
export async function deleteWorkoutExerciseGroup(input: { workoutId: string; exerciseId: string }) {
